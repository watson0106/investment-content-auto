...
def build_prompt(articles: list[dict], history_summary: str = "", strategy_state: dict = None) -> str:
    ...
    cta_text = """情報を正確に理解することは、投資の第一歩に過ぎない。株価は常に「情報の一歩先」を織り込んで動いており、ニュースを読むだけでは勝てないのが相場の現実だ。

私はこの先を「読み解く」だけでなく、「自分のお金でどう動くか」まで毎日メンバーシップで公開している。

▶ メンバーシップで読める内容（¥980/月・初月無料）
　・毎朝7時の注目銘柄2本を、買い水準・損切り・利確目安まで具体的な数字付きで配信
　・毎週月曜の実トレード全記録（先週の損益・勝率・反省点まで全公開）
...
# ①[ソース名]が報じたこと
[**冒頭の最初の1〜2文は「私が今朝○○した」「私はこのニュースを見て○○と判断した」など、自分の行動・判断から書き始めること**。第三者の報道引用「○○新聞が報じた」で始めるのは禁止。
そのうえで「[ソース名]によると、」と具体的な数字・変動率・規模を含む300〜400字の事実段落につなげる。見出しなしで直接本文から始める]
