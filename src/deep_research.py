...
def build_cta_block(recommended_stocks: list[dict] | None = None) -> str:
    ...
    recommended_stocks がある場合、各銘柄の有料記事リンクプレースホルダーを挿入"""
    stock_links = ""
    if recommended_stocks:
        ...
        for s in recommended_stocks:
            ...
            name = s.get("name", "")
            reason = s.get("reason", "")
            # プレースホルダー: main.py が有料記事投稿後に実URLで置換する
            lines.append(f"- **{name}（{ticker}）**の売買戦略 → <!-- paid_link:{ticker} -->[有料記事で詳しく見る](https://note.com/kawasewatson0106)<!-- /paid_link:{ticker} -->")
        stock_links = "\n" + "\n".join(lines) + "\n"
...
- その日のニュースに関連する具体的な銘柄を2〜3個、ティッカーシンボル付きで記事中に自然に言及すること（固定銘柄ではなくニュース内容から導き出される銘柄）
...
def run_deep_research(articles: list[dict], history_summary: str = "") -> dict:
    ...
    # 推奨銘柄がある場合、CTAブロックを銘柄プレースホルダー付きに差し替え
    if recommended_stocks:
        generic_cta = build_cta_block()  # 引数なし = 汎用CTA
        stock_cta = build_cta_block(recommended_stocks)
        if generic_cta.strip() in draft:
            draft = draft.replace(generic_cta.strip(), stock_cta.strip())
        else:
            # CTAが見つからない場合は末尾に追加
            draft = draft.rstrip() + "\n" + stock_cta
