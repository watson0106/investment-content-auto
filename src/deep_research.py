...
このニュースの概要・背景・意味合いを丁寧に解説する（1000字程度）。

【わかりやすさのルール（最重要）】
- 読者は投資に興味を持ち始めたばかりの一般人を想定する。難しい概念は**必ず身近な例え**で言い換えること
  - 例：「金利が上がるとは、銀行にお金を預けるだけで増える時代が戻ってきたようなもの」
  - 例：「TSMCの熊本工場とは、世界最高峰の料理人が日本に店を出すようなもの」
  - 例：「株価が織り込む、とはすでに噂が広まって値段に反映されている状態のこと」
- 「なぜそうなるのか」の因果関係を、例えや比喩を使って一段階ずつ丁寧に説明する
- 専門用語・業界用語が出たら**必ずカッコ内か直後に平易な説明**を添える（例：「3nm（髪の毛の約3万分の1の細さで回路を刻む最先端技術）」「EPS（1株あたり利益）」）
- ニュース概要セクション以外（H2見出し・銘柄セクション）でも、読者がつまずきそうな箇所には同様の例えを積極的に挿入する
...
【銘柄選定の基準】
- 買い（ロング）・空売り（ショート）どちらでも構わない。相場環境・ニュースの性質に応じてケースバイケースで判断する
- 「このニュースで株価が上がりやすい銘柄」ではなく、「このニュースで最も利益を大きく取れる銘柄」を選ぶ
- 相場が不安定・株高局面では、下落が見込まれる銘柄を空売りで取り上げることも積極的に行う
- 空売りを取り上げる場合は「空売り狙い」と明記し、下落シナリオと根拠を説明する

### [銘柄名（証券コード or ティッカー）]

- 本日の株価（土日は「週明け注目水準（直近終値）：〇〇円前後」）
- 買い狙いか空売り狙いかを最初に明示する（例：「このニュースは〇〇にとって逆風で、空売り優位と見ている」）
- このニュースとこの銘柄の直接的な関係を、**一般人にもわかる言葉**で説明する（専門用語が出たら必ずカッコ内か直後に平易な説明を添える）
- なぜその方向に動くと見るかは**さわりだけ**触れる（詳細な根拠・シナリオは有料noteに任せる）
- 「続きは有料noteで」という流れで自然に締める
（300〜400字程度）
