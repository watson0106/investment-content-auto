...
6. **「私が」「私は」を入れて一人称感を出す**
   - 「私が今週仕込んだ3銘柄」「私が日本株7：米国株3にした根拠」
   - 第三者解説ではなく、当事者の判断であることを明示する

【厳禁テンプレ（最近使いすぎ・絶対に使わない）】
- 「なぜXは急騰したのか／本当の理由／意外な関係」
- 「Xが急浮上させる○○」
- 「Xを呼ぶメカニズム」
- 「Xが動く本当の理由／裏事情／真相」
- 「Xを打ち破る○○」
これらの文型は読者から「またか」と思われ既にスキが減衰している。

【禁止タイトル】
- 「今日の投資ニュース速報」系（汎用的すぎてクリックされない）
- 「〜まとめ」「〜解説」だけのシンプルすぎるもの
- 40字超え
- 「？」で終わる修辞疑問だけのタイトル（自分の答えを先に出す）
...
タイトル候補のみを出力してください。"""
    ...
    # Windows/Mac両対応で claude フルパス解決
    import shutil
    claude_path = shutil.which("claude") or shutil.which("claude.cmd")

    text = ""
    if claude_path:
        print(f"  Claude CLI でタイトル生成中... ({claude_path})")
