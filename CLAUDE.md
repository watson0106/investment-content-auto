...
2. ② Gemini で深掘り記事執筆（解説に徹する、投資判断は書かない）
3. ③ Claude Opus で添削（ファクトチェック・個人ブロガー口調に調整）
4. ④ タイトル生成（Claude CLI → スコアリング自動選択）
5. ⑤ note に無料記事を下書き保存（画像なし）
6. ⑥ 投稿結果の検証（タイトル・本文の品質チェック）
7. ⑦ 有料記事生成・投稿（100円、画像なし、GS級分析＋ブロガー口調）
8. ⑧ 無料記事の末尾に有料記事リンクを追記
9. ⑨ PDCAトラッカー（スキ数更新）
